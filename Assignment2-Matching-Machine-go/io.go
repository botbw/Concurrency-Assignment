...
func readInput(conn net.Conn) (in input, err error) {
	...
	nullIndex := strings.Index(string(tmp), "\x00")
	in.instrument = string(tmp[:nullIndex])
	// in.instrument = *(*string)(unsafe.Pointer(&tmp))

	return
}

func outputOrderDeleted(in input, accepted bool, inTime, outTime int64) {
	acceptedTxt := "A"
	if !accepted {
		acceptedTxt = "R"
	}
...
func outputOrderAdded(in input, inTime, outTime int64) {
	orderType := "S"
	if in.orderType == inputBuy {
		orderType = "B"
	}
	fmt.Printf("%v %v %v %v %v %v %v\n",
		orderType, in.orderId, in.instrument, in.price, in.count, inTime, outTime)
}

func outputOrderExecuted(restingId, newId, execId, price, count uint32, inTime, outTime int64) {
	fmt.Printf("E %v %v %v %v %v %v %v\n",
